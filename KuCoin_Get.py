...
def load_KuCoin():
    ...
    for symbol in price_list['data']['ticker']:
        ...
        cursor.execute(f"INSERT INTO `price` (`symbol`, `price`) VALUES ('{symbol['symbol']}', '{symbol['last']}') ON DUPLICATE KEY UPDATE price = '{symbol['last']}' , last_update = UNIX_TIMESTAMP();") # Записать изменение цены
